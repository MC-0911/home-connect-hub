  ...
  return <section className="py-20 sm:py-28 bg-muted/30">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        duration: 0.5
          ...
          <Link to="/resources" className="group flex items-center gap-2 text-accent font-medium mt-4 sm:mt-0 hover:underline">
            View All Resources
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {articles.map((article, index) => <motion.div key={article.slug} initial={{
          opacity: 0,
          y: 20
        }} whileInView={{
          opacity: 1,
              ...
              <Link to={`/resources/${article.slug}`} className="block h-full">
                <Card className="group h-full overflow-hidden border-0 shadow-md hover:shadow-xl transition-shadow duration-300">
                  <div className="relative overflow-hidden">
                    <img src={article.image} alt={article.title} className="w-full h-52 object-cover transition-transform duration-500 group-hover:scale-105" />
