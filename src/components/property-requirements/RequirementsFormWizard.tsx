...
const stepInfo = [
  { icon: Home, label: 'Basics' },
  { icon: MapPin, label: 'Location' },
  { icon: Star, label: 'Features' },
  { icon: Calendar, label: 'Contact' },
];

const RequirementsFormContent = () => {
  const { formData, currentStep, setCurrentStep, totalSteps, resetForm } = useRequirementsForm();
    ...
    switch (step) {
      ...
      case 2:
        if (formData.minBudget && formData.maxBudget && 
          ...
          toast({ title: "Minimum budget cannot exceed maximum budget", variant: "destructive" });
          return false;
        }
        return true;
      case 3:
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setIsSubmitted(true);
      toast({ title: "Your requirements have been submitted successfully!" });
    } catch (error: any) {
