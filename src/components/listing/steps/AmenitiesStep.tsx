...
const AmenitiesStep = () => {
  ...
  const toggleArrayItem = (
    field: "flooring" | "rooms" | "indoorFeatures" | "amenities",
    item: string
  ) => {
    ...
    <div className="space-y-6">
      <div className="text-center mb-6">
        ...
        <p className="text-muted-foreground mt-1">
          Let's get into the nitty-gritty of your home's interior. What makes it special inside?
        </p>
      </div>
      ...
      <div className="space-y-8">
        ...
        <div className="space-y-2">
          ...
          <Select
            ...
            onValueChange={(value) => updateFormData({ basement: value })}
          >
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {flooringOptions.map((option, index) => (
              ...
              >
                <div
                  onClick={() => toggleArrayItem("flooring", option)}
                  className={`flex items-center gap-2 p-3 rounded-lg border cursor-pointer transition-all ${
                    formData.flooring.includes(option)
                      ? "border-primary bg-primary/5"
                      : "border-border hover:border-primary/50"
                  }`}
                >
                  <Checkbox
                    checked={formData.flooring.includes(option)}
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {roomOptions.map((option, index) => (
              ...
              >
                <div
                  onClick={() => toggleArrayItem("rooms", option)}
                  className={`flex items-center gap-2 p-3 rounded-lg border cursor-pointer transition-all ${
                    formData.rooms.includes(option)
                      ? "border-primary bg-primary/5"
                      : "border-border hover:border-primary/50"
                  }`}
                >
                  <Checkbox
                    checked={formData.rooms.includes(option)}
                    className="pointer-events-none"
                  />
                  <span className="text-sm">{option}</span>
                </div>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {indoorFeatureOptions.map((option, index) => (
              <motion.div
                key={option}
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
              ...
              >
                <div
                  onClick={() => toggleArrayItem("indoorFeatures", option)}
                  className={`flex items-center gap-2 p-3 rounded-lg border cursor-pointer transition-all ${
                    formData.indoorFeatures.includes(option)
                      ? "border-primary bg-primary/5"
                      : "border-border hover:border-primary/50"
                  }`}
                >
                  <Checkbox
                    checked={formData.indoorFeatures.includes(option)}
                    className="pointer-events-none"
                  />
                  <span className="text-sm">{option}</span>
                </div>
