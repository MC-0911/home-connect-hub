...
const ImagesStep = () => {
  ...
  return <div className="space-y-6">
      ...
      <div className="relative">
        <input type="file" accept="image/*" multiple onChange={handleFileSelect} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" id="image-upload" />
      ...
      {/* Image Preview Grid */}
      {formData.imagePreviewUrls.length > 0 && <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            <AnimatePresence mode="popLayout">
              {formData.imagePreviewUrls.map((url, index) => <motion.div key={url} layout initial={{
            opacity: 0,
            scale: 0.8
          }} animate={{
            opacity: 1,
            ...
            scale: 0.8
                  ...
                  <img src={url} alt={`Property image ${index + 1}`} className="w-full h-full object-cover rounded-lg border border-border" />
                  
                  {/* Cover Badge */}
                  {index === 0 && <div className="absolute top-2 left-2 bg-primary text-primary-foreground text-xs px-2 py-1 rounded-full">
                      Cover
                    </div>}
      ...
      {formData.imagePreviewUrls.length === 0 && <div className="rounded-lg p-4 text-center bg-sky-50">
