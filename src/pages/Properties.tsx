...
export default function Properties() {
  ...
  const filteredProperties = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      result = result.filter(p => p.title.toLowerCase().includes(query) || p.city.toLowerCase().includes(query) || p.state.toLowerCase().includes(query) || p.address.toLowerCase().includes(query));
    }
  ...
  const FilterContent = () => <div className="space-y-6">
      ...
      <div>
        ...
        <div className="flex rounded-full p-1 bg-sidebar-foreground">
          ...
          <button type="button" onClick={() => setListingType("sale")} className={`flex-1 px-4 py-2 text-sm font-medium rounded-full transition-colors ${listingType === "sale" ? "bg-primary text-primary-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"}`}>
            Buy
          </button>
          <button type="button" onClick={() => setListingType("rent")} className={`flex-1 px-4 py-2 text-sm font-medium rounded-full transition-colors ${listingType === "rent" ? "bg-primary text-primary-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"}`}>
            Rent
          </button>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="flex items-center gap-2">
          <Input type="number" placeholder="Min" value={minPrice} onChange={e => setMinPrice(e.target.value)} className="h-10" />
          <span className="text-muted-foreground">-</span>
          <Input type="number" placeholder="Max" value={maxPrice} onChange={e => setMaxPrice(e.target.value)} className="h-10" />
        </div>
      </div>
      ...
      <div>
        ...
        <div className="flex rounded-full p-1 bg-sidebar-foreground">
          ...
          value: "5",
          label: "5+"
        }].map(option => <button key={option.value} type="button" onClick={() => setBedrooms(option.value)} className={`flex-1 px-3 py-2 text-sm font-medium rounded-full transition-colors ${bedrooms === option.value ? "bg-primary text-primary-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"}`}>
              {option.label}
            </button>)}
        </div>
      </div>
      ...
      <div>
        ...
        <div className="flex rounded-full p-1 bg-sidebar-foreground">
          ...
          value: "4",
          label: "4+"
        }].map(option => <button key={option.value} type="button" onClick={() => setBathrooms(option.value)} className={`flex-1 px-3 py-2 text-sm font-medium rounded-full transition-colors ${bathrooms === option.value ? "bg-primary text-primary-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"}`}>
              {option.label}
            </button>)}
        </div>
      </div>
      ...
      <div>
        ...
        <div className="space-y-3 max-h-48 overflow-y-auto">
          {amenitiesList.map(amenity => <label key={amenity} className="flex items-center gap-3 cursor-pointer">
      ...
      {activeFiltersCount > 0 && <Button variant="outline" className="w-full" onClick={clearFilters}>
          Clear All Filters
        </Button>}
    </div>;
  return <div className="min-h-screen bg-background">
      ...
      <main className="pt-24 pb-16">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 bg-primary-foreground text-secondary">
          {/* Page Header */}
          <motion.div initial={{
          opacity: 0,
          y: 20
          ...
          delay: 0.1
            ...
            <div className="relative flex-1">
              ...
              <Input placeholder="Search by location, city, or address..." className="pl-12 h-12" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
            </div>
              ...
              <Sheet>
                <SheetTrigger asChild>
                  <Button variant="outline" className="lg:hidden h-12 relative">
                    ...
                    Filters
                    {activeFiltersCount > 0 && <Badge className="absolute -top-2 -right-2 w-5 h-5 p-0 flex items-center justify-center bg-accent text-accent-foreground">
                        {activeFiltersCount}
                      </Badge>}
                  </Button>
                </SheetTrigger>
          ...
          {/* Active Filters */}
          {activeFiltersCount > 0 && <motion.div initial={{
          opacity: 0,
          height: 0
        }} animate={{
          opacity: 1,
          height: "auto"
              ...
              {bedrooms && <Badge variant="secondary" className="gap-1">
                  {bedrooms}+ Beds
                  <X className="w-3 h-3 cursor-pointer" onClick={() => setBedrooms("")} />
                </Badge>}
              {bathrooms && <Badge variant="secondary" className="gap-1">
                  {bathrooms}+ Baths
                  <X className="w-3 h-3 cursor-pointer" onClick={() => setBathrooms("")} />
                </Badge>}
              {selectedAmenities.map(a => <Badge key={a} variant="secondary" className="gap-1">
                  {a}
                  <X className="w-3 h-3 cursor-pointer" onClick={() => setSelectedAmenities(selectedAmenities.filter(x => x !== a))} />
                </Badge>)}
            </motion.div>}
          ...
          <div className="flex gap-8">
            ...
            <div className="flex-1">
              {filteredProperties.length === 0 ? <motion.div initial={{
              opacity: 0
            }} animate={{
              opacity: 1
                ...
                </motion.div> : <div className={viewMode === "grid" ? "grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6" : "space-y-4"}>
